...
class CommentConfig:
    ...
    2. Project config (.spider-config.json codeScanning)
    ...
    def from_codebase_entry(
        ...
        Args:
            ...
            project_config: Optional codeScanning section from .spider-config.json
...
def scan_directory(
    ...
    """Scan a directory for code files with Spider markers.
    ...
    Returns:
        List of CodeFile objects (only files with Spider markers)
    """
    ...
    for ext in extensions:
        for path in directory.glob(f"{pattern}{ext}"):
            if not path.is_file():
                continue
            cf, errs = CodeFile.from_path(path)
            if cf and (cf.scope_markers or cf.block_markers):
                results.append(cf)
