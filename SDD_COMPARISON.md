# FDD vs OpenSpec vs Spec Kit vs MCAF vs BMAD vs Ralph
...
| Capability | FDD | OpenSpec | Spec Kit | MCAF | BMAD | Ralph |
|---|---|---|---|---|---|---|
...
| **✅ Deterministic doc/schema validator (format/placeholders/required fields)** | ✅ Native (`fdd validate ...`) | ✅ Native (`openspec validate ... --strict`) | ⚠️ Supported (templates + structured analysis/checklists; not a strict doc validator) | ❌ Out of scope | ❌ Out of scope | ❌ Out of scope |
...
| **🏷️ Deterministic code traceability validator (design/spec → code markers)** | ✅ Native (scans for `@fdd-*` tags) | ❌ Out of scope | ❌ Out of scope | ❌ Out of scope | ❌ Out of scope | ❌ Out of scope |
...
| **🤖 Agent instructions single source** | ✅ Native (`AGENTS.md` hierarchy) | ✅ Native (`openspec/AGENTS.md` + root hand-off) | ⚠️ Supported (slash commands + generated repo files) | ✅ Native (`AGENTS.md` governance + local AGENTS.md) | ✅ Native (specialized agents are core) | ⚠️ Supported |
| **⚙️ Repeatable automation packages ("skills")** | ✅ Native (FDD skill tool + scripts) | ⚠️ Supported (CLI + agent instructions) | ⚠️ Supported (scripts/templates) | ✅ Native (skills packages with scripts/references/assets) | ✅ Native (workflow library/modules) | ⚠️ Supported |
...
4. **Spec Kit has broad "Supported" coverage** (14 capabilities) but fewer "Native" features (3), indicating a generalist approach with lighter enforcement.

5. **Ralph scores lower on artifact/validator breadth** (21 points) because it is primarily an autonomy/loop technique; its strongest “native” features are verification gates, fresh-context loops, and iteration/stop conditions.
...
### Pattern D: BMAD for planning and artifact generation, then FDD/OpenSpec for long-lived specs
- Use **BMAD** to generate planning artifacts and maintain story-level execution flow.
- Use **FDD** (layered design + deterministic validation) and/or **OpenSpec** (delta change proposals) when you need long-lived, auditable specs with stronger format/validator guarantees.
...
- Pair with **FDD/OpenSpec/Spec Kit/BMAD** for planning/spec artifacts; treat Ralph as the “executor.”
